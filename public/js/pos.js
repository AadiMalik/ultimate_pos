        ...
        submitHandler: function (form) {
            ...
            if (cnf) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.success == 1) {
                            // 🔹 Elavon payment redirect (same tab)
                            if (result.redirect_url) {
                                window.location.href = result.redirect_url;
                            }

                            // 🔹 Print invoice (new tab)
                            if (result.print_url) {
...
$(document).on('change', '.payment_types_dropdown', function (e) {
    ...
    if (payment_type && payment_type != 'advance') {
        ...
        if (default_accounts) {
            ...
            // --- If payment method exists in mapping ---
            ...
            // --- Fallback: if elavon or any method missing → use cash ---
            else if (default_accounts['cash'] && default_accounts['cash']['account']) {
                default_account = default_accounts['cash']['account'];
            }
        }
